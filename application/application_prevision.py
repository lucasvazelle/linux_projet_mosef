...
st.markdown("""
    ...
    color: #333;  /* Texte foncé pour contraster avec le fond clair */
    padding: 2rem;
    border-radius: 10px;
...
def show_legend_and_info(col_legend):
    """Affiche la légende et les informations dans une colonne spécifiée"""
    # Dans la colonne de légende, ajouter une section d'informations
    with col_legend:
        # Ajouter la section d'informations
        st.markdown("""
        <div class="info-section">
        <h4 style='color: white; margin-bottom: 1rem;'>Guide d'utilisation</h4>
        <p class="info-text">🎯 Sélectionnez une année et une ville pour visualiser les données.</p>
        <p class="info-text">🗺️ Sur la carte :</p>
        <ul class="info-text">
        <li style='color: #AED6F1;'>🔵 Position de la ville</li>
        <li style='color: #AED6F1;'>🎨 Données (couleur selon le risque)</li>
        </ul>
        <p class="info-text">📊 Le graphique montre l'évolution des risques dans le temps.</p>
        </div>
        """, unsafe_allow_html=True)
...
<ul class="info-text">
<li style='color: #AED6F1;'>🔵 City location</li>
<li style='color: #AED6F1;'>🎨 Data (color according to risk level)</li>
</ul>
...
""", unsafe_allow_html=True)
    ...
    st.markdown('<div class="risk-badge risk-medium">Moderate Risk (2)</div>', unsafe_allow_html=True)
    ...
    with st.expander("ℹ️ About the application"):
        ...
        - Mariam Tarvedian [💼](https://www.linkedin.com/in/mariam-tarverdian-9a6140200)
...
with col_main:
    ...
    if city_name and country_name:
        ...
        if city_lat and city_lon:
            ...
            risk_text = "Low" if risk_level == 1 else "Moderate" if risk_level == 2 else "High" if risk_level == 3 else "Unknown"
            ...
            st.write("### Full Dataset for these Coordinates:")

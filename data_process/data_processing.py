...
wind_file_path = os.path.join(
    script_dir,
    "24_extreme_wind_speed_days-projections-yearly-rcp_4_5-wrf381p-ipsl_cm5a_mr-r1i1p1-grid-v1.0.nc",
)
precipitation_file_path = os.path.join(
    script_dir,
    "15_frequency_of_extreme_precipitation-projections-yearly-rcp_4_5-wrf381p-ipsl_cm5a_mr-r1i1p1-grid-v1.0.nc",
)
...
wind_df = wind_df[
    (wind_df["year"] >= 2025) & (wind_df["year"] <= 2035)
]  # Filtrage des années 2025-2035
...
print(
    f"📊 Percentiles calculés pour wind : 33% = {percentiles_wind[0.33]}, 66% = {percentiles_wind[0.66]}"
)
...
precipitation_df = precipitation_df[
    (precipitation_df["year"] >= 2025) & (precipitation_df["year"] <= 2035)
]  # Filtrage des années 2025-2035
...
print(
    f"📊 Percentiles calculés pour precipitation : 33% = {percentiles_precip[0.33]}, 66% = {percentiles_precip[0.66]}"
)

# Application de la fonction sur les jours dans le dataset precipitation
precipitation_df["risk"] = precipitation_df["days"].apply(
    lambda x: assign_risk(x, percentiles_precip)
)
